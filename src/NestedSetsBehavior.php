<?php
/**
 * @link https://github.com/songlipeng2003/yii2-mongodb-nested-sets
 * @copyright Copyright (c) 2015 Thinking Song
 * @license http://opensource.org/licenses/BSD-3-Clause
 */

namespace songlipeng2003\nestedsets;
 ...
 *
 * @author Alexander Kochetov <songlipeng2003@gmail.com>
 */
class NestedSetsBehavior extends Behavior
{
    ...
    protected function deleteWithChildrenInternal()
    {
        ...
        $condition = [
            'AND',
            $this->leftAttribute => [
                '$gte' => $this->owner->getAttribute($this->leftAttribute)
            ],
            $this->rightAttribute => [
                '$lte' => $this->owner->getAttribute($this->rightAttribute)
            ]
            // 'and',
    ...
    public function parents($depth = null)
    {
        $condition = [
            'AND',
            $this->leftAttribute => [
                '$lt' => $this->owner->getAttribute($this->leftAttribute)
            ],
            $this->rightAttribute => [
                '$gt' => $this->owner->getAttribute($this->rightAttribute)
            ]
            // 'and',
            // ['<', $this->leftAttribute, $this->owner->getAttribute($this->leftAttribute)],
            // ['>', $this->rightAttribute, $this->owner->getAttribute($this->rightAttribute)],
    ...
    public function children($depth = null)
    {
        $condition = [
            ...
            [
                $this->leftAttribute => [
                    '$gt' => $this->owner->getAttribute($this->leftAttribute)
                ]
            ],
            [
                $this->rightAttribute => [
                    '$lt' => $this->owner->getAttribute($this->rightAttribute)
                ]
            ]
            // 'and',
            // ['>', $this->leftAttribute, $this->owner->getAttribute($this->leftAttribute)],
